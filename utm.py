...
def utm_parse(utm_str: str) -> tuple:
    ...
    # Handle alternative formats with optional elevation: "315428mE 5741324mN 33N 1234m" 
    ...
    m = re.match(r'(\d+\.?\d*)\s*[Ee]\s*,?\s*(\d+\.?\d*)\s*[Nn]\s*,?\s*(\d+)\s*([NS])(?:\s*,?\s*(\d+\.?\d*)\s*[Mm]?)?', utm)
    if m:
        zone = int(m.group(3))
        if zone < 1 or zone > 60:
            raise UtmException(tr('Invalid UTM Coordinate'))
        ...
        # Ignore elevation (m.group(5)) if present
        return zone, hemisphere, easting, northing
