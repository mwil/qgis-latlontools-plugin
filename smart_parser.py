...
from qgis.core import (
    ...
    QgsPointXY, QgsJsonUtils, QgsMessageLog, Qgis
)
...
class CoordinateParserStrategy(ABC):
    ...
    def _log_debug(self, message: str):
        ...
        QgsMessageLog.logMessage(f"{self.__class__.__name__}: {message}", "LatLonTools", Qgis.Info)
...
class WkbParserStrategy(CoordinateParserStrategy):
    ...
    def can_parse(self, text: str) -> bool:
        ...
        return (len(text_clean) >= 20 and 
                len(text_clean) % 2 == 0 and 
                all(c in '0123456789ABCDEFabcdef' for c in text_clean))
    ...
    def parse(self, text: str) -> tuple:
        ...
        try:
            ...
            if not geom.isEmpty() and geom.type() == QgsWkbTypes.PointGeometry:
                pt = geom.asPoint()
                lat, lon = pt.y(), pt.x()
                self._log_debug(f"WKB parsed with QGIS: {text[:20]}... → lat={lat}, lon={lon}")
    ...
    def _manual_wkb_parsing(self, hex_string: str) -> tuple:
        ...
        try:
            ...
            self._log_debug(f"Need {coord_size} bytes for coordinates, have {len(wkb_bytes) - offset} remaining")
                
            if len(wkb_bytes) < offset + coord_size:
                self._log_debug(f"Insufficient bytes for coordinates: need {coord_size}, have {len(wkb_bytes) - offset}")
                return None
...
class GeohashParserStrategy(CoordinateParserStrategy):
    ...
    def can_parse(self, text: str) -> bool:
        ...
        return (bool(re.match(r'^[0-9bcdefghjkmnpqrstuvwxyz]+$', text_clean)) and
                3 <= len(text_clean) <= 12)
    ...
    def parse(self, text: str) -> tuple:
        ...
        if (re.match(r'^\d{1,2}[A-Z]{3}\d+$', re.sub(r'\s+', '', text_upper)) or  # MGRS
            re.match(r'^[A-Z]{4}\d{2,}$', text_upper) or  # GEOREF
            re.match(r'^[A-R]{2}\d{2}([A-X]{2}(\d{2})?)?$', text_upper)):  # Maidenhead
            return None
...
class GeoJsonParserStrategy(CoordinateParserStrategy):
    ...
    def can_parse(self, text: str) -> bool:
        ...
        return (text_stripped.startswith('{') and
    ...
    def _is_obviously_not_dms(self, text: str) -> bool:
        ...
        if len(numbers) >= 2:
            try:
                ...
                has_dms_indicators = any(indicator in text_upper for indicator in 
                    ['°', "'", '"', 'DEG', 'MIN', 'SEC', 'N', 'S', 'E', 'W'])
...
class SmartCoordinateParser:
    """
    Smart coordinate parser using Strategy Pattern
    ...
    def parse(self, text):
        """
        Main parsing entry point using try-parse strategy pattern
        ...
        try:
            # Try each strategy in specificity order until one succeeds
            for strategy in self.strategies:
                QgsMessageLog.logMessage(f"SmartParser.parse: Trying {strategy.__class__.__name__}", "LatLonTools", Qgis.Info)
                result = strategy.parse(text)
                if result is not None:
                    lat, lon, bounds, source_crs, description = result
                    QgsMessageLog.logMessage(f"SmartParser.parse: SUCCESS with {strategy.__class__.__name__} - {description}: lat={lat}, lon={lon}", "LatLonTools", Qgis.Info)
                    return (lat, lon, bounds, source_crs)
            
            # Fall back to existing coordinate parsing if no strategy matches
            QgsMessageLog.logMessage("SmartParser.parse: === FALLBACK: Trying existing coordinate formats ===", "LatLonTools", Qgis.Info)
            result = self._extract_and_validate_coordinates(text)
            if result:
                lat, lon, bounds, source_crs = result
                QgsMessageLog.logMessage(f"SmartParser.parse: SUCCESS with existing formats: lat={lat}, lon={lon}", "LatLonTools", Qgis.Info)
                return result
            
            QgsMessageLog.logMessage(f"SmartParser.parse: FAILED - No parser could handle input: '{original_text}'", "LatLonTools", Qgis.Warning)
            return None
            
        except Exception as e:
            QgsMessageLog.logMessage(f"SmartParser.parse: ERROR - {e}", "LatLonTools", Qgis.Critical)
            return None
