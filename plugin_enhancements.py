...
class PluginEnhancements:
    ...
    def enhance_settings_dialog(self, settings_widget):
        ...
        settings_widget.zoomToProjIsSmartAuto = lambda: (
            settings_manager.is_smart_auto_detect(settings_widget.zoomToProjection)
        )
        ...
        def enhanced_set_enabled():
            ...
            settings_manager.update_coordinate_order_enable_state(
                settings_widget.zoomToProjection
            )
    ...
    def enhance_zoom_dialog(self, zoom_dialog):
        ...
        # NOTE: The convertCoordinate method in zoomToLatLon.py now uses an optimized
        # smart parser path. No monkey-patching needed - the built-in method handles
        # all formats (DD, DMS, MGRS, UTM, Plus Codes, Geohash, H3, etc.)
        ...
        def enhanced_configure():
            ...
            if zoom_dialog.settings.zoomToProjIsSmartAuto():
                if zoom_dialog.settings.zoomToCoordOrder == 0:  # Y,X (Lat/Lon)
                    zoom_dialog.label.setText(
                        tr("Enter coordinates (auto-detected, prefers Lat/Lon order)")
                    )
                else:  # X,Y (Lon/Lat)
                    zoom_dialog.label.setText(
                        tr("Enter coordinates (auto-detected, prefers Lon/Lat order)")
                    )
        ...
        def enhanced_xy_button_clicked():
            preservation_mixin.preserve_text_during_order_change(
                zoom_dialog.coordTxt, zoom_dialog.settings, zoom_dialog.configure
            )

        zoom_dialog.xyButtonClicked = enhanced_xy_button_clicked

        # Add safe cleanup method
        zoom_dialog.cleanup = lambda: SafeDockWidgetCleanup.cleanup_zoom_dialog(
            zoom_dialog
        )
