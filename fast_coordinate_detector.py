...
COORDINATE_PATTERNS = {
    ...
    "dms_symbols": re.compile(
        r"[\u00b0\u2032\u2033\'\"°′″]"
    ),  # Degree, minute, second symbols
    "dms_letters": re.compile(
    ...
    "mgrs": re.compile(
        r"^\d{1,2}[C-HJ-NP-X][A-HJ-NP-Z][A-HJ-NP-V][0-9]{1,10}$", re.IGNORECASE
    ),
    "utm": re.compile(r"^\d{1,2}[NS]\s+\d{5,7}\.?\d*\s+\d{6,8}\.?\d*", re.IGNORECASE),
    "ups": re.compile(r"^[AB][A-HJ-NP-Z][0-9]{5,14}$", re.IGNORECASE),
    "plus_codes": re.compile(
        r"^[23456789CFGHJMPQRVWX]{4,8}\+[23456789CFGHJMPQRVWX]{2,3}", re.IGNORECASE
    ),
...
# Noise characters to sanitize (replace with space) - handles markdown tables, etc.
NOISE_CHARS_PATTERN = re.compile(
    r"[|\t\r]+"
)  # Pipe from markdown tables, tabs, carriage returns
...
class FastCoordinateDetector:
    ...
    def detect_format_fast(self, text: str) -> str:
        ...
        """
        # Sanitize noise characters from markdown tables, etc.
        # Replace pipes, tabs, carriage returns with spaces
        text_clean = NOISE_CHARS_PATTERN.sub(" ", text.strip())
