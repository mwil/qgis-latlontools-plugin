...
class LatLonToEcefAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.PrmXFieldName, tr("Output X attribute name"), defaultValue="X"
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
                self.PrmYFieldName, tr("Output Y attribute name"), defaultValue="Y"
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
                self.PrmZFieldName, tr("Output Z attribute name"), defaultValue="Z"
            )
        )
        self.addParameter(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if (
            self.PrmAltitudeField not in parameters
            or parameters[self.PrmAltitudeField] is None
        ):
            alt_field = None
        else:
            alt_field = self.parameterAsString(
                parameters, self.PrmAltitudeField, context
            )
        ...
        default_altitude = self.parameterAsDouble(
            parameters, self.PrmDefaultAltitude, context
        )
        ...
        (sink, dest_id) = self.parameterAsSink(
            parameters, self.PrmOutputLayer, context, xyzfields, QgsWkbTypes.NoGeometry
        )

        # ECEF transformation using QGIS built-in CRS support
        # EPSG:4979 = WGS 84 (3D) - geographic with height
        # EPSG:4978 = WGS 84 / geocentric - ECEF coordinates
        epsg4979 = QgsCoordinateReferenceSystem("EPSG:4979")
        epsg4978 = QgsCoordinateReferenceSystem("EPSG:4978")
        ...
        for cnt, feature in enumerate(iterator):
            ...
            # Create a 3D point with altitude for ECEF transformation
            pt3d = QgsPoint(pt.x(), pt.y(), alt)

            # Transform from layer CRS to EPSG:4979 if needed
            if layerCRS != epsg4979:
                to_4979 = QgsCoordinateTransform(
                    layerCRS, epsg4979, QgsProject.instance()
                )
                pt3d = to_4979.transform(pt3d)

            # Transform from geographic 3D to ECEF geocentric
            to_ecef = QgsCoordinateTransform(epsg4979, epsg4978, QgsProject.instance())
            ecef_pt = to_ecef.transform(pt3d)

            x, y, z = ecef_pt.x(), ecef_pt.y(), ecef_pt.z()
            f = QgsFeature()
...
class EcefLatLonToAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        self.addParameter(
            QgsProcessingParameterFeatureSource(
                self.PrmInputLayer, tr("Input layer"), [QgsProcessing.TypeVector]
            )
        )
        self.addParameter(
            QgsProcessingParameterField(
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.PrmOutputLayer, tr("Output pointZ layer")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        z_name = self.parameterAsString(
            parameters, self.PrmAltitudeFieldName, context
        ).strip()
        ...
        if add_z:
            if fieldsout.append(QgsField(z_name, QVariant.Double)) is False:
                raise QgsProcessingException(
                    tr("Altitude attribute name must be unique")
                )
        ...
        for cnt, feature in enumerate(iterator):
            ...
            # Create ECEF point and transform to geographic 3D
            ecef_pt = QgsPoint(x, y, z)
            to_geo = QgsCoordinateTransform(epsg4978, epsg4979, QgsProject.instance())
            geo_pt = to_geo.transform(ecef_pt)

            lon, lat, alt = geo_pt.x(), geo_pt.y(), geo_pt.z()
            pt = QgsPoint(lon, lat, alt)
            f = QgsFeature()
            if add_z:
