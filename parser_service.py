...
class CoordinateParserService:
    ...
    **Thread Safety Assumptions:**
    - Class-level _instance is set once during initialization
    - In QGIS, plugins are typically loaded in the main thread, so race conditions
      during initial creation are unlikely
    - LazyClassLoader uses threading.Lock() for double-checked locking
    - Service is designed for read-heavy use (parse operations)
    - Not safe for concurrent writes (create multiple instances simultaneously)
    - Fork-safe: reset_instance() can be called to clean up state after fork
    ...
    **Error Handling Strategy:**
    ...
    - Silent failures use Qgis.Debug level to avoid spamming production logs
    ...
    **Modification Points:**
    ...
    def _cleanup_parser_loader(cls, instance):
        ...
        if hasattr(instance, "_parser_loader") and instance._parser_loader:
            try:
                ...
                cls._log_cleanup_warning(
                    f"Failed to reset parser loader during cleanup: {e}"
                )
    ...
    def _log_cleanup_warning(cls, message):
        ...
        try:
            ...
            pass  # Intentionally silent - logging system may be unavailable
    ...
    def parse_coordinate_simple(self, text: str, component_name: str = "Unknown"):
        ...
        """
        success, result, error_msg = self.parse_coordinate_with_logging(
            text, component_name
        )
    ...
    def get_performance_stats(self):
        ...
        if self._optimized_parser is None:
            return {
                ...
                "message": "Optimized parser not yet initialized",
            }
...
class CoordinateParserMixin:
    ...
    **Modification Points:**
    ...
    def parse_coordinate_with_fallback(
        self, text: str, component_name: str, legacy_parser_func=None
    ):
        """
        ...
        success, result, error_msg = parser_service.parse_coordinate_with_logging(
            text, component_name
        )
        ...
        if success:
            ...
            try:
                QgsMessageLog.logMessage(
                    f"ParserMixin: {component_name} trying legacy fallback...",
                    "LatLonTools",
                    Qgis.Info,
                )
                ...
                if legacy_result:
                    QgsMessageLog.logMessage(
                        f"ParserMixin: {component_name} legacy fallback SUCCESS",
                        "LatLonTools",
                        Qgis.Info,
                    )
                    ...
                    QgsMessageLog.logMessage(
                        f"ParserMixin: {component_name} legacy fallback also failed",
                        "LatLonTools",
                        Qgis.Warning,
                    )
            except Exception as e:
                QgsMessageLog.logMessage(
                    f"ParserMixin: {component_name} legacy fallback exception - {e}",
                    "LatLonTools",
                    Qgis.Critical,
                )
    ...
    def parse_coordinate_simple_with_fallback(
        self, text: str, component_name: str, legacy_parser_func=None
    ):
        """
        ...
        Returns:
            (lat, lon, bounds, source_crs) or None
        """
        return self.parse_coordinate_with_fallback(
            text, component_name, legacy_parser_func
        )
...
def parse_coordinate_with_service(
    text: str, component_name: str, settings, iface, legacy_parser_func=None
):
    """
